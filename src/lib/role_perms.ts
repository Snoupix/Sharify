...
export function role_to_hierarchy(perms: Nullable<Role> | undefined) {
	return (
		Object.entries(perms?.permissions ?? {}) as Array<[keyof RolePermission, RolePermission[keyof RolePermission]]>
	)
		.filter(([, has]) => has)
...
export function sort_roles(room_data: Nullable<Room>) {
	return (
		room_data?.roleManager?.roles.sort((a, b) => {
...
export function can_user_moderate_user(room_data: Nullable<Room>, mod: Nullable<RoomUser>, target: RoomUser) {
	...
	if (
		!mod_role.permissions?.canManageUsers &&
		!mod_role.permissions?.canManageRoom &&
		!mod_role.permissions?.canAddModerator
	) {
		return false;
	}
