<script lang="ts">
    ...
    let sorted_roles = $derived(sort_roles($room_data));
    ...
    let perms_iter = $derived(
        (Object.keys(new_role.permissions) as Array<keyof RolePermission>)
            .filter(k => {
    ...
    <div class="create-role">
        <Select.Root
            ...
            onValueChange={(selected_perms) => {
                for (const perm of Object.keys(new_role.permissions) as Array<keyof RolePermission>) {
                    new_role.permissions[perm] = selected_perms.includes(perm);
                }
            }}>
            <Select.Trigger>
                <Button
            ...
            <div class="role">
                ...
                <div class="right">
                    {#if can_user_manage_role($room_data, current_user, role)}
                        <!-- TODO: Role Perms => Make a snippet of the thing above ? -->
                        {#if role_rename.i === i}
