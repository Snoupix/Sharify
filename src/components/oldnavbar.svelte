<script lang="ts">
	...
	import { get_storage_value, set_theme, type LocalStorage } from "$/lib/utils";
...
<nav>
	...
	<div>
		{#if profile != ""}
			<Button.Root onclick={disconnect}>
				<svg class="mr-2 w-5 fill-main-content" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 496 512"
		...
		{/if}
		<Select.Root
			items={[{ label: "Theme", value: theme }]}
			type="single"
			onValueChange={(e) => {
				set_theme((e as LocalStorage["theme"]) ?? null);
			}}>
			<Select.Trigger class="xl:w-32">
				<span class="font-montserrat text-base font-medium text-main-content">Theme</span>
			</Select.Trigger>
			<Select.Content class="select_content">
				<Select.Item class="select_item" value="purple">Purple</Select.Item>
				<Select.Item class="select_item" value="light">Light</Select.Item>
				<Select.Item class="select_item" value="dark">Dark</Select.Item>
				<Select.Item class="select_item" value="neon">Neon</Select.Item>
			</Select.Content>
		</Select.Root>
		<Button.Root>
		...
		<Button.Root>About</Button.Root>
		<Button.Root>Contact</Button.Root>
		{#if is_logged_in()}
			<Button.Root onclick={() => signOut()}>Logout</Button.Root>
		{:else}
			<Select.Root
				items={[{ label: "Login", value: login_method ?? "" }]}
				type="single"
				onValueChange={(e) => {
					if (e != undefined) {
						switch (e) {
				...
				<Select.Trigger class="xl:w-32">
					<span class="font-montserrat text-base font-medium text-main-content">Login</span>
				</Select.Trigger>
...
<style lang="postcss">
	...
	nav {
		...
		@apply relative z-10 border-b border-purple-500/30 bg-linear-to-br from-purple-800/40 to-purple-900/40 p-4 backdrop-blur-sm transition-all duration-500;

		div {
			@apply flex w-auto flex-row items-center justify-between gap-6 p-4;
	...
	:global(.login_btns .select_item > svg) {
		@apply m-auto w-7 fill-main-content stroke-main-content;
	}
</style>
