			...
			{:else}
				<div class="player_container">
					...
					{#key current_user}
						...
						{/if}
					{/key}
					<div class="h-[2px] w-full rounded-full bg-main"></div>
				</div>
												...
												{#if current_user != null && user.id != current_user.id && (get_user_role(current_user.id)?.permissions?.canManageUsers ?? false)}
													<div>
														<CustomButton onclick={() => kick_user(user)}
															>Kick</CustomButton>
														<CustomButton onclick={() => ban_user(user)}>Ban</CustomButton>
				...
				{#if search_input.trim() != "" && search_results.length != 0}
					<div class="results_wrapper">
						<div class="h-[2px] w-6/12 rounded-full bg-main"></div>
...
<style lang="postcss">
		...
		> div {
			...
			.bottom_container {
				...
				.clients {
					...
					.client {
						@apply flex flex-row items-center justify-center gap-2 rounded-full bg-main-hover px-4 py-1;
			...
			.player_controls {
				...
				:global(.round) {
					@apply m-0 h-10 w-10 rounded-full border border-main bg-transparent p-0 transition-all duration-300 hover:border-none hover:bg-main-hover;
	...
	:global(.client_card) {
		...
		* {
			@apply text-main-content dark:text-main-clear;
		}
	}
</style>
