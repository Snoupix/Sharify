<script lang="ts">
	...
	const room_data: Writable<Room | null> = getContext("RoomData");
    ...
    let is_ws_connected = $state(false);
	...
	function get_user_role(user_id: RoomUser["id"]): Nullable<Role> {
		...
		return $room_data?.roleManager?.roles.find((r) => sum_arr(r.id) === sum_arr(current_user!.roleId)) ?? null;
	}
	...
	onMount(async () => {
		...
		init_ws(room_id, user_id, () => is_ws_connected = true, () => is_ws_connected = false, on_ws_error, on_ws_message);
...
<section>
	{#if leaving || $ws === null || !is_ws_connected}
        {is_ws_connected}
        {$ws?.readyState}
        {$ws?.OPEN}
		<div class="loading">
