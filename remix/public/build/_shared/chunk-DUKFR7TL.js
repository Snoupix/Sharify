...
if (!window.$RefreshReg$ || !window.$RefreshSig$ || !window.$RefreshRuntime$) {
  ...
  window.$RefreshReg$ = (type, id) => {
    window.$RefreshRuntime$.register(type, '"app/utils/utils.tsx"' + id);
  };
...
if (import.meta) {
  ...
  import.meta.hot.lastModified = "1712663496724.9104";
}
var SetStorageValue = (value) => {
  if (!localStorage || !window.localStorage)
    throw new Error("Cannot access localStorage nor window.localStorage");
  const store = localStorage != null ? localStorage : window.localStorage;
  ...
  store.setItem("Sharify", JSON.stringify({
    ...parsed_storage,
    ...value
  }));
  return;
...
function GetStorageValue(value) {
  if (!localStorage || !window.localStorage)
    return {};
  const store = localStorage != null ? localStorage : window.localStorage;
  const store_object = JSON.parse(store.getItem("Sharify") || "{}");
  const returns = store_object[value];
...
function FormatTime(progress, duration) {
  const [p2, m2] = [new Date(progress), new Date(duration)];
  const FormatNumber = (int) => {
    return int.toString().padStart(2, "0");
  };
  return `${FormatNumber(p2.getMinutes())}:${FormatNumber(p2.getSeconds())} / ${FormatNumber(m2.getMinutes())}:${FormatNumber(m2.getSeconds())}`;
}
_c3 = FormatTime;
var _c;
var _c2;
var _c3;
$RefreshReg$(_c, "SetStorageValue");
$RefreshReg$(_c2, "GetStorageValue");
$RefreshReg$(_c3, "FormatTime");
...
//# sourceMappingURL=/build/_shared/chunk-DUKFR7TL.js.map
