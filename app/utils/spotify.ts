...
export default class SpotifyHandler {
    ...
    private static SetTokens() {
        const tokensStorage = GetStorageValue("st")
        const tokens = tokensStorage as SpotifyTokensStorage

        if (!tokens) {
            console.error('Spotify tokens not found', tokensStorage, tokens)
            return
        }
...
class SpotifyHandler {
    ...
    public currentDevice: SpotifyApi.UserDevice | undefined
    public isReady = false

    public SetCredentials(creds: { id: string, secret: string }) {
        SpotifyPlayer = new SpotifyAPI({
            clientId: creds.id,
            clientSecret: creds.secret,
            redirectUri: "http://localhost:3000/auth_spotify"
    ...
    private CallAuthorizationAPI(body: string) {
        fetch("https://accounts.spotify.com/api/token", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
    ...
    public ProcessTokens(data: SpotifyTokens) {
        if (this.timeout) {
            clearTimeout(this.timeout)
        }
    ...
    GetRecentlyPlayedTracks(limit: number) {
        this.SetTokens()
        return new Promise<Array<SpotifyApi.PlayHistoryObject | Error>>((resolve, reject) => {
            SpotifyPlayer
                .getMyRecentlyPlayedTracks({ limit: limit })
